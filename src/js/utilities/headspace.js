function Headspace(element, opts = {}) {
  ...
  this.startOffset = optionOrDefault(
    opts.startOffset,
...
Headspace.prototype = {
  init() {
    ...
    window.addEventListener('scroll', () =>
      this.debounce(() => handleScroll(this))
    );
  },
  reset() {
...
function handleScroll(instance) {
  ...
  if (scrollCurrent <= 0) {
    ...
    instance.showAtBottom &&
    window.innerHeight + scrollCurrent >= document.body.offsetHeight
  ) {
    instance.fix();
  } else if (
    scrollCurrent > instance.startOffset &&
    Math.abs(scrollCurrent - scrollLast) >= instance.tolerance
  ) {
