...
if (!function_exists('ab_custom_login_logo')):
  function ab_custom_login_logo()
  {
    ...
    $theme_logo_url = $theme_logo_id
      ? wp_get_attachment_url($theme_logo_id)
      : __t() . 'public/img/default-logo-black.svg';

    echo '<style type="text/css">
  h1 a { background-image: url(' .
      $theme_logo_url .
      ') !important; height: 80px!important; width:300px!important; background-size: contain!important;}
...
if (!function_exists('ab_remove_dashboard_widgets')):
  function ab_remove_dashboard_widgets()
  {
    ...
    // Remove right now
    unset(
      $wp_meta_boxes['dashboard']['normal']['core']['dashboard_getshopped_news']
    );
    unset(
...
function ab_customize_register($wp_customize)
{
  ...
  $wp_customize->add_control(
    new WP_Customize_Media_Control($wp_customize, 'custom_logo_white', array(
      'label' => __('White Logo'),
      'description' => esc_html__(
        'Upload an optional white version of the logo to be used on non-white backgrounds'
      ),
   ...
   * Social Accounts
   */
  $wp_customize->add_section('social_accounts', array(
  ...
  $wp_customize->add_section('analytics', array(
    ...
    'description' => esc_html__(
      'Add your Analytics ID below. It will be used in the tag manager script in the header of the page'
    )
  ));
  ...
  $wp_customize->add_control('google_analytics_id', array(
    'label' => __('Google Analytics Account ID'),
    'section' => 'analytics',
    'input_attrs' => array(
      // Optional.
      'placeholder' => __('ex. UA-1234567-1')
    )
  ));
}
